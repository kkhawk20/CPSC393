...
class VideoDataGenerator(Sequence):
    ...
    def display_video_sequence(self, index):
        # Fetch a batch of videos
        X, _ = self.__getitem__(index)
        video_sequence = X[0]  # Take the first video in the batch

        fig, axes = plt.subplots(1, self.max_frames, figsize=(15, 2))
        for i, frame in enumerate(video_sequence):
            axes[i].imshow(frame)
            axes[i].axis('off')
        plt.savefig("test_video_sequence.png")
...
train_gen.display_video_sequence(0)
...
def build_generator(hp):
    ...
    # Expand the input data to match the spatial dimensions expected by ResNet50
    x = layers.Dense(256 * 256 * 3, activation='relu')(concat_input)
    x = layers.Reshape((1, 256, 256, 3))(x)

    # Apply ResNet50 model within TimeDistributed to extract features
...
if retrain:
    ...
    generator = load_weights("generator_weights_tuned.h5", compile=False)
